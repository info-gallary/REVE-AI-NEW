...
def create_evaluation_graphs(
    ...
    # Plot per-class lightly to avoid clutter; still useful for paper appendix
    for i in range(n_classes):
    ...
    # Per-class (optional, can be many lines)
    for i in range(n_classes):
    ...
    width = 0.25
    fig, ax = plt.subplots(figsize=(max(8, 0.6 * n_classes), 5), dpi=160)
